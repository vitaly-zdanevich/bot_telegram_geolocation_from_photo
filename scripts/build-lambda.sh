...
BIN_NAME="telegram-photo-geolocator-bot"
...
install_local_rustup() {
	...
	case "$arch" in
		...
		*) echo "Unsupported host architecture for automatic rustup install: $arch" >&2; exit 1 ;;
	esac
...
echo "Building Lambda binary for $TARGET with target-cpu=$RUST_TARGET_CPU"
...
if [[ ! -f "$zip_candidate" ]]; then
	zip_candidate="$(find "$LAMBDA_DIR" -maxdepth 4 -type f \( -name bootstrap.zip -o -name '*.zip' \) | sort | head -n 1)"
fi
